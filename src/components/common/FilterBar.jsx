...
const FilterBar = ({ activeFilter, onFilterChange, leads = [] }) => {
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full">
      <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider transition-colors duration-200">
        Filter Stage:
      </span>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-semibold whitespace-nowrap cursor-pointer transition-all duration-200 border ${
                isActive
                  ...
                  : 'bg-white dark:bg-gray-800 border-slate-200 dark:border-gray-700 text-slate-600 dark:text-gray-300 hover:bg-slate-50 dark:hover:bg-gray-700 hover:text-slate-800 dark:hover:text-white'
              }`}
            >
              {status}
              <span
